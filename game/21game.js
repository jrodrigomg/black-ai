...
{
	enviroment.on("startGame",startGame);
	enviroment.on("getState", getState);
...
function getState()
{
	//Creamos un array de 13 figuras de 4 suits y los llenamos de 0 
	//Esto debido a que es one hot encoding hecho
	// let state = new Array(13*4).fill(0);
	// let hand = playerHand.getHand();
	// hand.forEach(function(card) 
	// {
	// 	let figure = card.getNumber();
	// 	let suit = card.getSuit();
	// 	let index = (figure - 1 ) + ((suit - 1) * 13);
	// 	state[index]= 1;
	// });

	state = playerHand.score() - 1;

	//Ver si ha ganado para poner el reward bastante grande.
	//O ver si perdio para poner el reward negativo
	if(isFinished)
	{
		let finalScore = finalResultCheck();
		switch(finalScore)
		{
			case "Tide": //Si es empate se lleva el 10%
				score = 0.6;
				break;
			case "Bust": //Si perdio entonces es -10%
				score = -0.2;
				break;
			case "Win": //Si gano entonces se lleva un 1 :3
				score = 1;
				break;
			default:
				score = 0.1;//No se porque llega aqui pero pongo cualquier valor
		}
	}else
	{
		//El score será el score / 21 ya que lo más que puede tener para que no termine el juego es 21
		score = playerHand.score() / 22;
	}
	
	data = {"state":state, "reward":score, "finished":isFinished}
	console.log("Enviando data:", data)
	enviroment.sendData(JSON.stringify(data))  
}
